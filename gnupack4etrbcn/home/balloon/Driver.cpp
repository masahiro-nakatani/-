...
#include "MissionCourseG1.cpp"
#ifdef IN_COURSE
#include "MissionCourseG1_SeeSaw.cpp"
#else // OUT_COURSE
#include "MissionCourseG1_LookupGate.cpp"
#endif
...
TASK(TaskDrive){
	...
	Driver::Instance().AddMission(new MissionBalloon());
	//Driver::Instance().AddMission(new Mission_S_Turn());

	//Driver::Instance().AddMission(new MissionStart(Technic::RIGHT));
...
//	Driver::Instance().AddMission(new Mission_GarageIn());
	...
	// インコース: 後半シーソー.
	CourseInfo::type = CourseInfo::InCourse;
	Driver::Instance().AddMission(new Mission_CourseG1(0, 60, RobotCmd::NO_TAIL_CNTL, Technic::LEFT));
	Driver::Instance().AddMission(new Mission_CourseG1_SeeSaw(15000, 40, RobotCmd::NO_TAIL_CNTL, Technic::LEFT));
#else // OUT_COURSE
	// アウトコース: 後半ゲート.
	CourseInfo::type = CourseInfo::OutCourse;
	Driver::Instance().AddMission(new Mission_CourseG1(0, 60, RobotCmd::NO_TAIL_CNTL, Technic::LEFT));
	Driver::Instance().AddMission(new Mission_CourseG1_LookupGate(0, 70, RobotCmd::NO_TAIL_CNTL, Technic::LEFT));
#endif
#endif
