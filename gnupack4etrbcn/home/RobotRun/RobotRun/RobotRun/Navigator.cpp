...
NavInfo Navigator::GetInfo(const RobotInfo& r_info){
	...
	getPositionEstimation(info_.pos, r_info.rot_encoderR_val - r_info_.rot_encoderR_val
		, r_info.rot_encoderL_val - r_info_.rot_encoderL_val, TIRE_RADIUS, TIRE_DISTANCE, newPos);
	//最新のロボット情報を保持
	r_info_  = r_info;
	//位置情報を最新に更新
	info_.pos = newPos;
...
void Navigator::SetInfo (F32 x, F32 y, F32 ang) {
	info_.pos.X = x;
	info_.pos.Y = y;
	info_.pos.Ang = ang;
}

Position Navigator::GetNowPos (void) {
	return info_.pos;
}
