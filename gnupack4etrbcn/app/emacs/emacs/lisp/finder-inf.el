...
(setq finder-keywords-hash
      #s(hash-table size 487 test eq rehash-size 1.5 rehash-threshold 0.8 data (abbrev (skeleton pcomplete emacs icomplete hippie-exp expand edmacro dabbrev completion autoarg) convenience (diff-mode compare-w table picture emacs makeinfo flyspell erc keypad cua-base calc winner windmove vcursor tmm ruler-mode repeat minibuf-eldef mb-depth longlines linum kmacro json iswitchb imenu ido ibuffer hippie-exp follow filesets filecache ffap ebuff-menu desktop delsel dabbrev completion chistory calculator bs autorevert autoinsert align) languages (sgml-mode xscheme vhdl-mode verilog-mode vera-mode tcl sql simula sh-script scheme ruby-mode python ps-mode perl-mode pascal octave-mod modula2 mixal-mode meta-mode m4-mode ld-script js idlwave icon fortran flymake f90 executable delphi dcl-mode cwarn cperl-mode cfengine cc-mode cc-guess cap-words autoconf asm-mode antlr-mode ada-mode erc smie emacs tempo skeleton paren ledit info-look allout align) lisp (inversion cedet xscheme scheme inf-lisp hideshow unsafep trace testcover tabulated-list smie shadow re-builder pp map-ynp macroexp emacs ewoc ert elp elint eieio edebug debug checkdoc check-declare benchmark strokes isearchb ielm color cmuscheme align) outlines (hideshow hideif org reveal outline foldout elide-head allout allout-widgets) wp (underline two-column tildify emacs table sgml-mode refill picture page-ext nroff-mode ispell enriched ebnf2ps handwrite org whitespace ps-print printing longlines allout) pgp (gnus epg epa emacs allout) gnupg (gnus epg epa emacs allout) comm (url sgml-mode dns-mode sql zeroconf webjump tramp tls telnet socks soap-client secrets rlogin rcirc net-utils ldap gnutls eudc dns dig dbus ange-ftp erc bindat terminal talk shadowfile notifications master kermit ansi-color) processes (url sql inf-lisp idlwave grep compile tramp rcompile eshell erc bindat term shell server proced pcomplete emacs emacs-lock dirtrack cmuscheme ansi-color) terminals (tramp xt-mouse vt-control terminal ansi-color) services (ansi-color) help (emacs woman novice man makesum informat info info-look icomplete help-at-pt finder ehelp) files (compare-w url po wdired view emacs shadowfile recentf locate ffap doc-view arc-mode) archives (arc-mode) msdog (arc-mode) editing (hideshow dcl-mode arc-mode) major-mode (dcl-mode arc-mode) extensions (cookie1 gnus tq tabulated-list rx ring regi regexp-opt map-ynp ewoc eldoc emacs cust-print cl benchmark avl-tree assoc calc type-break tree-widget thingatpt tempo strokes skeleton ido emacs-lock electric ehelp array) emulations (ws-mode viper vip vi tpu-edt edt cua-base crisp emacs hl-line delsel autoarg) mouse (artist goto-addr browse-url xt-mouse emacs thingatpt t-mouse strokes msb mouse-sel mouse-drag mouse-copy ffap avoid) hardware (zeroconf dbus vt100-led emacs flow-ctrl battery) internal (emacs warnings syntax smie disass userlock select novice loadhist gs delim-col) bookmarks (saveplace bookmark) placeholders (saveplace bookmark) annotations (bookmark) tools (semantic vc smerge-mode log-view emerge ediff diff cvs-status add-log which-func make-mode hideshow gud grep glasses gdb-mi flymake etags ebrowse cpp compile bug-reference asm-mode xesam rcompile reporter mail-utils trace re-builder package ert elp eieio edebug debug copyright emacs check-declare time-stamp tempo speedbar skeleton sb-image imenu format-spec find-file ezimage elide-head dframe calculator) i18n (po emacs gnus double descr-text) data (url remember page-ext org snmp-mode gnus bindat timeclock xml whitespace jka-compr emacs hexl hex-util cdl) faces (enriched m4-mode ld-script cwarn cpp gnus erc emacs paren hl-line hilit-chg hi-lock face-remap descr-text color) color (color) hex (color) rgb (htmlfontify color) hsv (color) hsl (color) cie-lab (color) background (color) mule (emacs ps-print) multilingual (emacs descr-text) character\ composition (emacs) expand (dabbrev) completion (crm iswitchb dabbrev) unicode (emacs descr-text) file (dns-mode speedbar sb-image ezimage dframe) tags (semantic ebrowse speedbar sb-image ezimage dframe) dired (wdired emacs) window (emacs windmove follow) drag (emacs) drop (emacs) pdf (doc-view) ps (doc-view) dvi (doc-view) font (ps-print emacs) system-font (emacs) tool-bar-style (emacs) unix (emerge ediff diff ispell sh-script make-mode gud gdb-mi executable cc-mode telnet rlogin woman tar-mode sort proced emacs lpr locate find-lisp find-dired) mail (fortune mairix imap goto-addr mh-e unrmail undigest uce supercite smtpmail sendmail rmail rfc822 rfc2368 reporter mspools metamail mailpost mail-utils mailclient mailalias mailabbrev hashcash footnote feedmail emacs gnus time-date epa) message (feedmail gnus epa) face\ remapping (face-remap) display (hi-lock follow face-remap) user\ commands (face-remap) hypermedia (url sgml-mode css-mode org xesam soap-client quickurl browse-url emacs htmlfontify ffap) matching (regi re-builder thingatpt soundex misearch emacs hi-lock find-file ffap) filesets (filesets) c (hideshow hideif flymake cwarn cpp cmacexp cc-mode cc-guess emacs find-file) folding (foldout) minor-mode (hi-lock follow) frames (winner emacs talk hl-line) generic (emacs) comment (emacs) font-lock (emacs) colour (htmlfontify) html (gnus htmlfontify) markup (htmlfontify) etags (htmlfontify) buffer (msb ibuffer) multimedia (metamail gnus thumbs mpc emacs image-file image-dired iimage) docs (texnfo-upd texinfo texinfmt makeinfo lisp-mnt checkdoc info-xref) keyboard (keypad cua-base kmacro) md4 (md4) utilities (midnight) menu (erc msb) uncomment (emacs) desktop (notifications) notifications (notifications) password (secrets password-cache) cache (password-cache) passphrase (secrets password-cache) key (password-cache) print (handwrite ps-print printing) postscript (ps-mode ebnf2ps handwrite gnus ps-print printing) bdf (ps-print) multibyte (ps-print) vi (repeat) repeat (repeat) minibuffer (crm savehist emacs) scroll (scroll-all) crisp (crisp scroll-all) brief (crisp scroll-all) lock (scroll-all) spreadsheet (testcover ses) dijkstra (ses) gpm (t-mouse) linux (t-mouse) news (newsticker netrc uudecode supercite metamail mail-utils footnote binhex gnus time-date timezone) timers (type-break) virtual\ cursor (vcursor) environment (wdired) renaming (wdired) movement (windmove) xml (nxml xml) calendar (org gnus todo-mode timeclock calendar icalendar diary-lib) holidays (calendar) util (time-date parse-time) todo (remember todo-mode) maint (texnfo-upd texinfo texinfmt reporter emacs gulp edebug debug copyright checkdoc check-declare) data\ structures (avl-tree) avl (avl-tree) tree (cvs-status avl-tree) oo (inversion cedet eieio chart) chart (chart) graph (chart) multiple\ elements (crm) debugging (elp) emacs-lisp (find-func) functions (find-func) variables (find-func) compiler (macroexp) macros (macroexp) strings (rx regexp-opt) regexps (rx regexp-opt) parsing (smie) indentation (smie) utility (unsafep testcover) safety (unsafep testcover) cua (cua-base) mark (cua-base) irc (org erc) chat (erc) client (erc) internet (erc) button (erc) url (vc url erc) regexp (nxml erc) logging (erc) timestamp (erc) cancel-lock (gnus) hmac (hmac-md5 hmac-def gnus) sha1 (gnus) rfc2104 (hmac-md5 hmac-def gnus) web (gnus) xpm (gnus) annotation (gnus) glyph (gnus) registry (gnus) synchronization (gnus) nntp (gnus) nnrss (gnus) network (network-stream net-utils dns gnus) external-body (gnus) partial (gnus) uudecode (uudecode gnus) binhex (binhex gnus) shar (gnus) forward (gnus) gnatsweb (gnus) gnus (gnus) mime (metamail gnus) s/mime (gnus) mml (gnus) searching (mairix gnus) ir (gnus) rss (newsticker gnus) hz (gnus) hz+ (gnus) smime (gnus) x\.509 (gnus) pem (gnus) openssl (gnus) spam (mh-e rmail gnus) gmane (gnus) report (gnus) bogofilter (gnus) bbdb (gnus) dspam (gnus) dig (dig gnus) whitelist (gnus) blacklist (gnus) hashcash (hashcash gnus) spamassassin (gnus) bsfilter (gnus) ifile (gnus) stat (gnus) crm114 (gnus) spamoracle (gnus) tls (tls gnutls gnus) ssl (tls gnutls gnus) gnutls (tls gnus) yenc (gnus) ccl (emacs) character\ set (emacs) coding-system (emacs) multibyte\ character (emacs) syntax (semantic mode-local emacs) category (emacs) fontset (emacs) isearch (emacs) tex (tildify texnfo-upd texinfo texinfmt tex-mode reftex refbib bibtex bibtex-style meta-mode org emacs) iso (emacs) latin (emacs) japanese (emacs) kinsoku (emacs) coding\ system (emacs) latex (bibtex emacs) charset (emacs) diagnosis (emacs) input\ method (emacs) quail (emacs) tit (emacs) cxterm (emacs) normalization (emacs) burma (emacs) cham (emacs) chinese (emacs) cyrillic (emacs) czech (emacs) ethiopic (emacs) european (emacs) greek (emacs) korean (emacs) hanja (emacs) hebrew (emacs) indian (emacs) devanagari (emacs) khmer (emacs) lao (emacs) romanian (emacs) sinhala (emacs) slovak (emacs) tai\ viet (emacs) thai (emacs) word\ break (emacs) emacs (emacs) tibetan (emacs) vietnamese (emacs) email (rmail feedmail) queue (feedmail) sendmail (feedmail) spray (feedmail) smtp (feedmail) draft (feedmail) history (mail-utils) filter (rmail) rmail (rmail) uce (uce) unsolicited\ commercial\ email (uce) hypertext (browse-url) dns (dns-mode dig) bind (dig) encryption (gnutls) mh-e (goto-addr) www (webjump goto-addr) hmac-md5 (hmac-md5) md5 (hmac-md5) keyed-md5 (hmac-md5) cram-md5 (sasl hmac-md5) atom (newsticker) ntlm (sasl ntlm) sasl (sasl ntlm) digest-md5 (sasl) soap (soap-client) web-services (soap-client) firewalls (socks) relaxng (nxml) literate\ programming (org) reproducible\ research (org) r (org) statistics (org) comint (org) fortran (fortran f90 org) js (org) accounting (org) babel\ language (org) maxima (org) scheme (ede org) interactive\ shell (org) shen (org) org (org) task (org) remember (org) org-mode (org) docbook (org) erc (org) link (org) tables (org) plotting (org) emacsclient (org) taskjuggler (org) project\ planning (org) games (zone yow tetris studly spook solitaire snake pong mpuz morse meese life landmark hanoi gomoku gametree gamegrid fortune dunnet doctor dissociate decipher cookie1 bubbles bruce blackbox animate 5x5) puzzles (5x5) utils (fortune) cursive\ writing (handwrite) gomoku (landmark) neural\ network (landmark) adaptive\ search (landmark) chemotaxis (landmark) ada (ada-mode) project\ file (ada-mode) xref (ada-mode) antlr (antlr-mode) code\ generator (antlr-mode) awk (cc-mode) cc-mode (cc-mode) oop (cc-guess) perl (cperl-mode) dcl (dcl-mode) ebnf (ebnf2ps) c++ (hideshow ebrowse) f90 (f90) java (conf-mode hideshow) comments (hideshow) blocks (hideshow) hiding (hideshow) javascript (js) g++ (mantemp) templates (mantemp) metafont (meta-mode) metapost (meta-mode) knuth (mixal-mode) mix (mixal-mode) mixal (mixal-mode) asm (mixal-mode) mixvm (mixal-mode) the\ art\ of\ computer\ programming (mixal-mode) prolog (prolog) major (prolog) mode (prolog) sicstus (prolog) swi (prolog) mercury (prolog) ruby (ruby-mode) tcl (tcl) modes (tcl) vera (vera-mode) vhdl (vhdl-mode) mode-line (which-func) imenu (which-func) bib (refer refbib bib-mode) bibtex (bibtex) conf (conf-mode) ini (conf-mode) windows (conf-mode) master (dns-mode) zone (dns-mode) soa (dns-mode) memory (remember) pim (remember) text (tildify) sgml (tildify) vc (ede vc smerge-mode pcvs log-view log-edit emerge ediff diff diff-mode cvs-status compare-w add-log url) pcl-cvs (pcvs log-edit cvs-status) cvs (smerge-mode pcvs log-view log-edit cvs-status) status (cvs-status) patch (diff-mode) diff (diff-mode) comparing (ediff) merging (ediff) patching (ediff) commit (log-edit) log (log-view log-edit) rcs (log-view) sccs (log-view) release\ management (pcvs) revision\ control (smerge-mode) merge (smerge-mode) diff3 (smerge-mode) conflict (smerge-mode) project (semantic ede) make (semantic ede) codegeneration (srecode))))

(provide 'finder-inf)
;; Local Variables:
